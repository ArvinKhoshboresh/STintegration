...
import sys
import torch
from blackbox_backprop import perfect_matching
...
def lil_matrix_to_blossom(cost_matrix):

    num_vertices = cost_matrix.shape[0] + cost_matrix.shape[1]

    # Get the non-zero elements and their indices
    rows, cols = np.nonzero(cost_matrix)
    weights = cost_matrix[rows, cols].todense().A1

    # Create edges and edge_weights lists
    edges = list(zip(rows, cols + cost_matrix.shape[0]))
    edge_weights = list(weights)

    return edges, edge_weights, num_vertices
...
edges, edge_weights, num_vertices = lil_matrix_to_blossom(distances_matrix)
...
# Print the solution
print("Solution (Selected edges):", matches)
